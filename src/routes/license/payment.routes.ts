  ...
  fastify.post('/api/license/tripay-callback', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (status === 'PAID') {
      try {
        ...
        const existingLic = await prisma.license.findUnique({
        ...
        let days = 30;
        if (planId.includes('sem') || planId.includes('semester')) days = 180;
        else if (planId.includes('annual') || planId.includes('tahun') || planId.includes('yearly')) days = 365;
...
- *Status*: ✅ *LUNAS (Aktif s.d ${expiresStr})*`;
  ...
  fastify.get('/api/license/history-by-core-key/:coreKey', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (normalizeProductId(coreLicense.productId) === 'cakola') {
    ...
    try {
      ...
      if (inv.paymentInstructions) {
        try {
          ...
          if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
            if (typeof parsed.base_price === 'number') basePriceVal = parsed.base_price;
            if (typeof parsed.admin_fee === 'number') adminFeeVal = parsed.admin_fee;
          }
        } catch {}
      }

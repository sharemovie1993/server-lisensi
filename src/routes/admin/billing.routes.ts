...
import { prisma, normalizeProductId, formatWA } from '../license/helpers';
...
export async function processPrivateerTopUp(invoice: any) {
  const cleanPhone = formatWA(invoice.licenseId);
  ...
  await waGateway.sendMessage(cleanPhone, message, 'TOPUP_AUTO_CONFIRM', 'privateer');
}

export const registerBillingRoutes = (fastify: FastifyInstance) => {
  ...
  fastify.post('/api/admin/privateer/manual-topup', async (request: FastifyRequest, reply: FastifyReply) => {
    await verifyAdmin(request, reply);
    if (reply.sent) return;

    const { phone, studentName, sessions, price } = request.body as {
      phone: string;
      studentName: string;
      sessions: number;
      price: number;
  ...
  fastify.post('/api/admin/subscriptions/migrate', async (request: FastifyRequest, reply: FastifyReply) => {
    await verifyAdmin(request, reply);
    if (reply.sent) return;

    const { schoolName, targetLicenseId } = request.body as { schoolName: string; targetLicenseId: string };
  ...
  fastify.post('/api/admin/invoices/pay/:id', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      let days = 30;
      if (planId.toLowerCase().includes('tahun') || planId.toLowerCase().includes('annual') || planId.toLowerCase().includes('yearly')) days = 365;
      else if (planId.toLowerCase().includes('sem') || planId.toLowerCase().includes('semester')) days = 180;
      else if (planId.toLowerCase().includes('lifetime')) days = 3650;
  ...
  fastify.post('/api/admin/subscriptions/bulk-delete', async (request: FastifyRequest, reply: FastifyReply) => {
    await verifyAdmin(request, reply);
    if (reply.sent) return;

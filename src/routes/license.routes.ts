...
import { PRIVATE_KEY, PUBLIC_KEY, ADMIN_SECRET } from '../utils/keys';
import { logLicenseActivity } from '../utils/logger';

const prisma = new PrismaClient();

function formatWA(nomor: string): string {
  if (!nomor) return '';
...
async function verifyClient(request: FastifyRequest, reply: FastifyReply) {
...
export const licenseRoutes: FastifyPluginAsync = async (fastify: FastifyInstance) => {
  ...
  fastify.get('/api/license/check-slug/:slug', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.post('/api/license/tripay-callback', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (status === 'PAID') {
      try {
        ...
        let days = 30;
        if (planId.includes('semester')) days = 180;
        else if (planId.includes('annual')) days = 365;
        else if (planId.includes('lifetime')) days = 3650;
  ...
  fastify.get('/api/license/history-by-core-key/:coreKey', async (request: FastifyRequest, reply: FastifyReply) => {
    const { coreKey } = request.params as { coreKey: string };
    try {
  ...
  fastify.get('/api/license/print-invoice/:invoiceNumber', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const dateStr = inv.paidAt
        ? formatIndonesianDate(inv.paidAt.toISOString().slice(0, 10))
        : formatIndonesianDate(inv.createdAt.toISOString().slice(0, 10));
...
`;
  ...
  fastify.get('/api/license/easy-tunnel/check-vnc-port/:license_key', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const checkTcpPort = (ip: string, port: number, timeoutMs = 2500): Promise<boolean> => {
  ...
  fastify.get('/api/license/payment-channels', async (_request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.data)) {
        const mappedData = data.data.map((channel: any) => {
  ...
  fastify.post('/api/license/upload-receipt', async (request: FastifyRequest, reply: FastifyReply) => {
    const { license_key, image } = request.body as { license_key: string; image: string };
    ...
    try {
      ...
      await logLicenseActivity(license_key, license.productId, clientIp, 'RECEIPT_UPLOAD_SUCCESS');
  ...
  // 28. Download APK
  fastify.get('/download-apk', async (request: FastifyRequest, reply: FastifyReply) => {
    const fs = require('fs');
    const path = require('path');
  ...
  fastify.post('/api/license/easy-tunnel/update-port', async (request: FastifyRequest, reply: FastifyReply) => {
    const { license_key, local_port } = request.body as { license_key: string; local_port: number };

...
export const cohortService = {
  ...
  async getCohortRetention(db: PrismaClient, limit: number, productId?: string) {
    ...
    const tenantStartDates = new Map<string, Date>();
    ...
    for (const key of sortedCohortKeys) {
      ...
      for (const tenantId of tenantIds) {
        ...
        if (isActive) {
          if (monthsDiff >= 1) retained1++;
          if (monthsDiff >= 3) retained3++;
          if (monthsDiff >= 6) retained6++;
          if (monthsDiff >= 12) retained12++;
        }
      }

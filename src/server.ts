...
function initVpnFirewall(): void {
  ...
  try {
    ...
    exec('sudo iptables -C INPUT -i wg0 -p icmp -j ACCEPT 2>/dev/null || sudo iptables -I INPUT 1 -i wg0 -p icmp -j ACCEPT');
...
async function startServer() {
  ...
  triggerCaddySync().catch(err => console.error('[CADDY SYNC ERROR]', err));
  checkExpirations().catch(err => console.error('[STARTUP CRON ERROR]', err));

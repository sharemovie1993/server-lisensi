...
exports.registerUndanganDigitalLicenseRoutes = void 0;
 ...
 */
const registerUndanganDigitalLicenseRoutes = (fastify) => {
    /**
     * 1. GET /api/license/undangan-digital/packages
     * Mengambil semua paket aktif untuk produk undangan-digital
     */
    fastify.get('/api/license/undangan-digital/packages', async (_request, reply) => {
        try {
            // Pastikan produk undangan-digital ada di database
            let product = await helpers_1.prisma.product.findUnique({ where: { id: 'undangan-digital' } });
            if (!product) {
                product = await helpers_1.prisma.product.create({
                    data: {
                        id: 'undangan-digital',
            ...
            let plans = await helpers_1.prisma.plan.findMany({
            ...
            if (plans.length === 0) {
                ...
                for (const p of defaultPlans) {
                    await helpers_1.prisma.plan.create({ data: p });
                }
                plans = await helpers_1.prisma.plan.findMany({
                    where: { productId: 'undangan-digital', isActive: true },
                    orderBy: { priceMonthly: 'asc' }
                });
            }
     ...
     */
    fastify.post('/api/license/undangan-digital/create-order', async (request, reply) => {
        ...
        try {
            const plan = await helpers_1.prisma.plan.findUnique({ where: { id: plan_id } });
            if (!plan) {
            ...
            const formattedPhone = (0, helpers_1.formatWA)(customer_phone);
            if (formattedPhone && whatsapp_service_1.waGateway) {
                ...
                whatsapp_service_1.waGateway.sendMessage(formattedPhone, msg, 'INVOICE_CREATED', 'undangan-digital').catch(() => { });
            }
            return reply.send({
                success: true,
                data: {
     ...
     */
    fastify.get('/api/license/undangan-digital/check-status/:invoice_number', async (request, reply) => {
     ...
     */
    fastify.post('/api/license/undangan-digital/validate-license', async (request, reply) => {
     ...
     */
    const handleSendOtp = async (request, reply) => {
        const body = request.body;
        ...
        try {
            ...
            await whatsapp_service_1.waGateway.sendMessage(formattedPhone, textToSend, 'OTP_UNDANGAN', 'undangan-digital');
            return reply.send({
                success: true,
...
exports.registerUndanganDigitalLicenseRoutes = registerUndanganDigitalLicenseRoutes;

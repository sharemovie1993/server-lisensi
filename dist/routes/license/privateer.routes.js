...
exports.registerPrivateerLicenseRoutes = void 0;
const crypto_1 = __importDefault(require("crypto"));
const helpers_1 = require("./helpers");
const registerPrivateerLicenseRoutes = (fastify) => {
    /**
     * 1. Specialized endpoint for Privateer Top-up
     * Pattern: Parent License (School) -> Multiple Student Top-ups (Metadata in SchoolName)
     */
    fastify.post('/api/license/privateer/topup', async (request, reply) => {
        const { student_name, student_id, class_id, class_name, plan_id, payment_method, wa_number, license_key // Parent license key
         } = request.body;
        ...
        try {
            ...
            const parentLicense = await helpers_1.prisma.license.findUnique({
            ...
            (0, helpers_1.sendPrivateerTopUpNotification)(wa_number, student_name, class_name || 'Kursus Privat', plan.name, invoiceNumber, tx.amount, payment_method || 'QRIS2', 'unpaid', tx.pay_code, tx.qr_url).catch((e) => console.error('[WA Privateer Notify Error]', e.message));
            return reply.send({
                success: true,
                data: {
     ...
     */
    fastify.get('/api/license/privateer/sessions/:licenseKey', async (request, reply) => {

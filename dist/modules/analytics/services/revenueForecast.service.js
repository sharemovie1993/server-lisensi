...
exports.revenueForecastService = {
    async calculateAndUpsertForecast(db, targetMonth, productId) {
        const now = new Date();
        ...
        for (const r of risks) {
            ...
            if (r.riskLevel === 'HIGH_RISK')
                riskAdjustment += mrr * 0.3;
            if (r.riskLevel === 'CRITICAL')
                riskAdjustment += mrr * 0.6;
        }
    ...
    async getLatestForecast(db, productId) {
        const now = new Date();
        return this.calculateAndUpsertForecast(db, now, productId);
    },
    async lockMonthIfExists(_db, _month) {
        return true;
    }
};
